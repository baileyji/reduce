pro modeinfo_feros,mode,newhead,object,orient,xlo,xhi,ylo,yhi,gain,readn,backg,time $
                       ,obslat,obslon,obsalt,ra2000,de2000,jd,NONLINEAR=nonlinear
;===========================================================================================
;The ESO La Silla FEROS spectrometer at the 2.4m.
    if n_elements(orient) eq 0 then orient = 6
    if not keyword_set(xr) then xr = [50, 2097]
    xlo = xr(0)
    xhi = xr(1)
    if not keyword_set(yr) then yr = [3, 4098]
    ylo = yr(0)
    yhi = yr(1)
    ...
    if n_elements(ra2000) eq 0 then begin
      ra=hierarch(newhead,'RA-OBJ',count=count, errmes=errmes)
      ra=ra/3600.                        ; RA in hours
    endif

    if n_elements(de2000) eq 0 then begin
      dec=hierarch(newhead,'DE-OBJ',count=count, errmes=errmes)
    ...
    if n_elements(obslon) eq 0 then obslon = double(ten( 70,44,12.865)) ;observatory coordinates
    if n_elements(obslat) eq 0 then obslat = double(ten(-29,15, 7.422))
    if n_elements(obsalt) eq 0 then obsalt = 2335.
