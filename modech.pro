pro modech, file_or_struct, sig=sig, cont=cont, wave=wave, orders=orders $
...
; 08-Nov-2002 Mittermayer Allow for reverse absolute order numbers in 
;                         wavelength section
;
...
if n_params() lt 1 then begin
  if not keyword_set(syntax) then syntax = 'modech, file_or_struct'
  ...
  if keyword_set(process) and wtype ne 0 then begin	;true: need to process
    case wtype of
      ...
      2: begin                                          ;2-dim fits             
         ...
         if not keyword_set(norders) then norders = nord
         if not keyword_set(orders) then orders = indgen(nord) + wvc[3]
         if norders gt 1 then begin                     ;true: order list       
           ikeep = orders - wvc[3]                      ;list of orders to keep 
           if max(ikeep) ge nord then begin             ;true: bad order list   
             message, /cont, 'order list extends beyond spectrum - aborting'    
             return                                                             
           endif                                                                
           wave = wave[*,ikeep]                         ;keep desired orders    
         endif                                                                  
         wtype = 0                                      ;clear flag             
         end                                                                    
